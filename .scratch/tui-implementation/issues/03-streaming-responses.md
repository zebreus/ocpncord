...
- **`Event::Backend(BackendEvent)` variant**: Add to the existing `Event` enum. Carries a normalized `BackendEvent` from an `EventEnvelope`. The app driver polls `Backend::EventStream` from `subscribe_live()` alongside crossterm input and tick.
...
- **Stream state in App**:
  - `live_events: Option<B::EventStream>` — global live stream from `subscribe_live()`
  - `sync_known_sequences` — cursor map for `/sync/history` reconnect catch-up
  - `partial_parts: Vec<Part>` — accumulated from `BackendEvent::Part` events
  - On `BackendEvent::MessagePartUpdated` / `MessagePartDelta`: update `partial_parts`
  - On assistant `BackendEvent::MessageUpdated`: commit `partial_parts` as a new `LoadedMessage`, append/update `messages`
...
- **`submit_prompt()` call**: On Enter from the PromptBar, `App` calls `Backend::submit_prompt(session_id, text, agent)`, uses the immediate receipt only as acknowledgement, and reads live output from `subscribe_live()`.
...
- [ ] Sending a message calls `Backend::submit_prompt()` and keeps consuming the live stream
- [ ] `Event::Backend(BackendEvent::MessagePartUpdated)` / `MessagePartDelta` updates `partial_parts` and renders them in the Chat message list
- [ ] Assistant `Event::Backend(BackendEvent::MessageUpdated)` commits the partial message as a final message and renders it statically
...
- [ ] `cargo test -p ocpncord-tui` passes with tests for:
  ...
  - App stream handling: part updates/deltas → partial_parts, assistant MessageUpdated → commit, Error → display
