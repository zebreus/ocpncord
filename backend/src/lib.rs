...
pub enum BackendEvent {
    ...
    SessionError { session_id: SessionId, error: ServerError },
    SessionDiff { session_id: SessionId, diff: Vec<SnapshotFileDiff> },
    SessionCompacted { session_id: SessionId },
    MessageUpdated { session_id: SessionId, message: Message },
    MessageRemoved { session_id: SessionId, message_id: MessageId },
    MessagePartUpdated { session_id: SessionId, part: Part },
    MessagePartDelta { session_id: SessionId, message_id: MessageId, part_id: String, field: String, delta: String },
    MessagePartRemoved { session_id: SessionId, message_id: MessageId, part_id: String },
    PermissionAsked { request: PermissionRequest },
    PermissionReplied { session_id: SessionId, request_id: String, reply: String },
    QuestionAsked { request: QuestionRequest },
    QuestionRejected { session_id: SessionId, request_id: String },
    QuestionReplied { session_id: SessionId, request_id: String, answers: Vec<String> },
    CommandExecuted { name: String, session_id: SessionId, arguments: String, message_id: String },
    FileEdited { file: String },
    FileWatcherUpdated { file: String, event: String },
    PtyCreated { info: Pty },
    PtyUpdated { info: Pty },
    PtyDeleted { id: String },
    PtyExited { id: String, exit_code: i32 },
    ...
    TuiToastShow { message: String, variant: String, title: Option<String>, duration: Option<u64> },
    TuiSessionSelect { session_id: SessionId },
...
impl fmt::Display for BackendEvent {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SessionError { session_id, error } => write!(f, "session.error: {session_id}: {error:?}"),
            ...
            Self::MessageRemoved { session_id, message_id } => write!(f, "message.removed: {session_id}/{message_id}"),
            Self::MessagePartUpdated { session_id, .. } => write!(f, "message.part.updated: {session_id}"),
            Self::MessagePartDelta { session_id, message_id, part_id, .. } => write!(f, "message.part.delta: {session_id}/{message_id}/{part_id}"),
            Self::MessagePartRemoved { session_id, message_id, part_id } => write!(f, "message.part.removed: {session_id}/{message_id}/{part_id}"),
            Self::PermissionAsked { request } => write!(f, "permission.asked: {}", request.permission),
            Self::PermissionReplied { session_id, request_id, reply } => write!(f, "permission.replied: {session_id}/{request_id}: {reply}"),
            Self::QuestionAsked { request } => write!(f, "question.asked: {}", request.id),
            Self::QuestionRejected { session_id, request_id } => write!(f, "question.rejected: {session_id}/{request_id}"),
            Self::QuestionReplied { session_id, request_id, .. } => write!(f, "question.replied: {session_id}/{request_id}"),
            Self::CommandExecuted { name, session_id, .. } => write!(f, "command.executed: {name} ({session_id})"),
            Self::FileEdited { file } => write!(f, "file.edited: {file}"),
            Self::FileWatcherUpdated { file, event } => write!(f, "file.watcher.updated: {file} ({event})"),
            ...
            Self::LspDiagnostics { server_id, path } => write!(f, "lsp.diagnostics: {server_id}: {path}"),
            Self::LspUpdated => write!(f, "lsp.updated"),
            Self::McpBrowserOpenFailed { mcp_name, url } => write!(f, "mcp.browser.open.failed: {mcp_name}: {url}"),
            Self::McpToolsChanged { server } => write!(f, "mcp.tools.changed: {server}"),
            Self::InstallationUpdateAvailable { version } => write!(f, "installation.update-available: {version}"),
            ...
            Self::TuiToastShow { message, variant, .. } => write!(f, "tui.toast.show: [{variant}] {message}"),
            ...
            Self::ServerInstanceDisposed { directory } => write!(f, "server.instance.disposed: {directory}"),
        }
    }
}
